(function ($) {
    ...
    $.fn.slide=function (options) {
        ...
        for(var i=0;i<loadLength;i++){
            loadLi.push('<li class="oneImageBox2" data-order="'+i+'" id="theImgBox'+i+'"></li>');
        ...
        if(setting.height){
            ...
            if(imageWidth>800){
                $(".slideImageBox").height(imageWidth*0.7);
            }else{
                $(".slideImageBox").height(imageWidth*0.7);
            }
        }
...
//      }
        ...
        * */
         ...
         * ***图片与文字关联
         * */
        function exampleListShow() {
            var falg = $('#example-box ul li.oneImageBox').data('order');
            console.log(falg);
            $('.example-list li').hide();
            $('.example-list li').eq(falg).show();
        }
    }
})(jQuery);
